...
describe('paginator', () => {
	...
	it('Should return the next link when there is another page available', () => {
		const links = paginatorInstance({ total, offset, limit, resource });
		expect(links.next).toEqual(formatUrl(host, resource, offset + limit, limit));
	});
	...
	it('Should return the prev link when there is a previous page available', () => {
		...
		expect(links.prev).toEqual(formatUrl(host, resource, (testOffset - limit), limit));
	});
	...
	it('Should return the offset as 0 when there is a previous page available below the limit', () => {
		...
		expect(links.prev).toEqual(formatUrl(host, resource, 0, limit));
	});
});
