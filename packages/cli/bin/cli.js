...
const run = async () => {
	...
	program
		.version('0.1.0')
		.command('start', 'start an application server in the current directory', { executableFile: `${__dirname}/bmo-create.js` })
		.command('create', 'creates a BMO http application', { executableFile: `${__dirname}/bmo-create.js` })
		.command('add', 'adds a npm module to the bmo dependencies, and installs if it is not in the package.json', { executableFile: `${__dirname}/bmo-create.js` });
